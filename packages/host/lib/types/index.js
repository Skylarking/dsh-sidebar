...
import { WorkspaceConsoleSessionId, } from "./types.js";
class OutputTail {
    maxBytes;
    chunks = [];
    retainedBytes = 0;
    ...
    append(text) { this.appendText(text); }
    read(fromByte) {
    ...
    appendText(text) {
        ...
        while (this.retainedBytes > this.maxBytes) {
            ...
            if (first === undefined)
                throw new Error('workspace console output tail lost its retained chunk');
            ...
            if (first.byteLength <= excess) {
                ...
                let start = excess;
                while (start < first.byteLength && ((first[start] ?? 0) & 0xc0) === 0x80)
                    start += 1;
...
let WorkspaceConsoleGateway = (() => {
    ...
    return class WorkspaceConsoleGateway extends _classSuper {
        ...
        async open(workspaceId, cols, rows) {
            ...
            if (this.sessions.size + this.pendingOpens.size >= this.config.maxSessions)
                throw new Error('workspace console session limit reached');
        ...
        session(sessionId) {
            const record = this.sessions.get(sessionId);
            if (record === undefined)
                throw new Error(`unknown workspace console session '${sessionId}'`);
            return record;
        }
        assertActive() {
            if (this.disposing)
                throw new Error('workspace console is disposing');
        }
        assertDimensions(cols, rows) {
            if (!Number.isInteger(cols) || cols < 1 || cols > 1000)
                throw new Error('terminal columns must be an integer between 1 and 1000');
        ...
        async terminate(record) {
            if (record.outcome === undefined) {
                ...
                const graceful = await Promise.race([
                    ...
                    new Promise(resolvePromise => setTimeout(() => resolvePromise(false), this.config.graceMs)),
                ]);
...
export { WorkspaceConsoleGateway };
